sidfex.plot.speedangle <- function(index=NULL,read.fcst.res=NULL,read.obs.res=NULL,remap.res=NULL, plot.empty = F,
  ...
  if(!plot.empty){
    if (is.null(read.fcst.res)) {
      if (is.null(index)) {
        print("Warning: 'index' and 'read.fcst.res' are not specified - will create empty plot")
        plot.empty = T
      } else {
  ...
  plot(NA,xlim=pir$xlim,ylim=pir$ylim,xaxt="n",yaxt="n",bty="n")
