sidfex.read.fcst <- function(read.fcst.raw) {
          ...
          for (iy in InitYear) {
            for (idoy in InitDayOfYear) {
              for (emn in EnsMemNum) {
                ...
                filepathnames = c(filepathnames,system(paste0("ls ",file.path(data.path.fcst,GroupID,filestr)),intern=TRUE))
              }
            }
          }
        }
  ...
  if (ens.merge) {res.list = sidfex.ensmerge(res.list)}
  return(rl.merged)
